...
class Cesta_Compra extends Model
{
    ...
    {
        // Calcular el total de productos directamente desde la base de datos
        $totalProductos = $this->productos()->sum('cesta_productos.cantidad');

        if ($totalProductos === 0) {
            return $this->actualizarPorcentajesCero();
        }

        // Obtener niveles y contar productos por nivel en una sola consulta
